...
{
    public:
    virtual void IndexDirectory(const wstring& directory, int& fileCount, int& subdirectoryCount, unordered_map<string, wstring>& fileIndex) = 0;
};

// Derived class implementing btree-search method for directory indexing
class BtreeSearchIndexer : public DirectoryIndexer 
{
    public:
    void IndexDirectory(const wstring& directory, int& fileCount, int& subdirectoryCount, unordered_map<string, wstring>& fileIndex)
    {
